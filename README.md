...
The next version under development is `2.9.0` (build `22`). It adds adjustable screenshot selection, direct copy/save/OCR, screenshot pins and local document-to-Markdown conversion. The annotation editor now offers grouped tools with visible shortcut badges, a keyboard reference, contextual styles, zoom/pan and remembered drawing preferences. Style adjustments undo in one step, switching tools clears stale selections, and marker numbering follows undo/redo. Themes have distinct backgrounds and improved Midnight readability. See the [2.9.0 release notes](RELEASE_NOTES.md#290--screenshot-editing-and-document-conversion) and [screenshot controls](docs/SCREENSHOT_TOOLS.md).
