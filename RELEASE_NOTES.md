...
- Removed the white strip that macOS 26/27 paints across the top of every `NavigationSplitView` window. The settings, diary, chat history and music windows now keep a transparent titlebar, so the system stops drawing a titlebar background over the first lines of the detail column. Window size, title, toolbar and the sidebar toggle are unchanged.
- Screenshots no longer skip YuanGUI's own windows. Capture hides only the running session's selection overlays, matched by window number, so the pet, dashboard, settings, chat history, diary and music player show up in captures and can be picked in Window mode.
