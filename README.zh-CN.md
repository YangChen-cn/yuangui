...
从 **工具 → PDF 转 Markdown** 进入，首次安装转换组件后，选择或拖入一个 PDF 即可本地转换。可以预览、复制正文，或导出 Markdown 和配套 `_assets` 文件夹：插图（含矢量图）保留在正文对应位置。默认移除每页重复的页眉页脚；扫描件的自动 OCR 由窗口开关控制，默认关闭。复杂分栏和公式仍需人工整理。参见[转换说明与第三方声明](docs/PDF_CONVERSION.md)。
