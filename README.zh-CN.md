...
下一开发版本为 `2.9.0`（build `22`），新增可调整截图选区、直接复制/保存/识字、截图固定与本地文档转 Markdown。标注编辑器提供工具分组、可见快捷键与速查、上下文样式、缩放平移，以及绘图偏好记忆；连续样式调整可一次撤销，切换工具清除旧选中项，编号随撤销/重做恢复。各主题采用不同底色，并改善夜色可读性。详情见 [2.9.0 更新说明](RELEASE_NOTES.zh-CN.md#290--截图编辑与文档转换)和[截图操作说明](docs/SCREENSHOT_TOOLS.md)。
