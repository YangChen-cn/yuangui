...
下一开发版本为 `2.9.0`（build `22`），新增基于 PyMuPDF Layout 1.28.2 本地模型与可选自动中英文 OCR 的 PDF 转 Markdown、位图导出，并减少工具面板和手帐预览的重复刷新开销。详情见 [2.9.0 更新说明](RELEASE_NOTES.zh-CN.md#290--pdf-转-markdown)。

从 **工具 → PDF 转 Markdown** 进入，首次安装转换组件后，选择或拖入一个 PDF 即可本地转换。可以预览、复制正文，或导出 Markdown 和配套图片文件夹；扫描件的自动 OCR 由窗口开关控制，默认关闭。复杂分栏、公式和矢量图不会被完整还原，识别到的位图区域按页码附在文末。参见[转换说明与第三方声明](docs/PDF_CONVERSION.md)。
